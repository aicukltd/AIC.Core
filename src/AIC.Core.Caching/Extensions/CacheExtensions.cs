...
public static class CacheExtensions
{
    ...
    {
        return keys.Select(key => (TValue)cache.Get(key.ToString(), k => factory(key))).ToArray();
    }
    ...
    public static async Task<TValue> GetAsync<TKey, TValue>(this ICache cache, TKey key,
    ...
    public static async Task<TValue[]> GetAsync<TKey, TValue>(this ICache cache, TKey[] keys,
        Func<TKey, Task<TValue>> factory)
    {
